...
export default function EmployerDashboard() {
  ...
  const myJobsRaw = useQuery(api.jobs.getEmployerJobs, {});
  const myChallengesRaw = useQuery(api.challenges.getEmployerChallenges, {});
  const myJobs = myJobsRaw ?? [];
  const myChallenges = myChallengesRaw ?? [];
                ...
                {myJobs.map((job) => (
                  <div key={job._id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-cool-grey">{job.companyName || '—'}</p>
                    </div>
                    <div className="flex gap-2">
                      <Button variant="ghost" size="sm" onClick={() => navigate(`/employer/jobs/${job._id}/applicants`)}>
                        <Users className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => navigate(`/employer/jobs/${job._id}/edit`)}>
        ...
        <NeoCard className="p-6">
          ...
          <NeoCardContent>
            {myChallenges.length === 0 ? (
              ...
              <div className="space-y-3">
                {myChallenges.map((ch) => (
                  <div key={ch._id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                    <p className="font-medium text-charcoal">{ch.title}</p>
                    <div className="flex gap-2">
                      <Button variant="ghost" size="sm" onClick={() => navigate(`/employer/challenges/${ch._id}/submissions`)}>
                        <Users className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => navigate(`/employer/challenges/${ch._id}/edit`)}>
                        <Edit2 className="h-4 w-4" />
                      </Button>
                    </div>
                  </div>
