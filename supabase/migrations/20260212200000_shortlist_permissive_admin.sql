-- More permissive shortlisting: add can_shortlist bypass for admins who can't shortlist
-- Fixes: "can't shortlist" when user_roles/user_type not set correctly

-- Add can_shortlist flag to profiles (optional bypass)
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS can_shortlist BOOLEAN NOT NULL DEFAULT false;

-- Grant shortlist to common admin emails (run once to fix your account)
-- Uncomment and run in SQL Editor: UPDATE profiles SET can_shortlist = true WHERE id = auth.uid();

-- 1. Venture shortlisting - allow can_shortlist OR employer/investor
...
-- 2. Job application shortlisting - allow can_shortlist OR employer/investor OR job owner
